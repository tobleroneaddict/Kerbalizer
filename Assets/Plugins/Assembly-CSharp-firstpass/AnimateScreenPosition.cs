...
public class AnimateScreenPosition : EZAnimation
{
	...
	{
		base.WaitDone();
		if ((base.Mode == ANIM_MODE.By || base.Mode == ANIM_MODE.To) && subSP != null)
		{
	...
	public override bool Start(GameObject sub, AnimParams parms)
	{
		...
		{
			Start(sub, parms.mode, eZScreenPlacement.renderCamera.WorldToScreenPoint(eZScreenPlacement.transform.position), parms.vec, EZAnimation.GetInterpolator(parms.easing), parms.duration, parms.delay, null, parms.transition.OnAnimEnd);
		}
		return true;
	}
